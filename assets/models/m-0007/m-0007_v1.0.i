...
interactor tray
 ...
 axioms
  ...
  [ac:{opent, closet}] keep(empty)
  [putPaper] !empty'
  [opent] openStatus'
  [closet] !openStatus'
  [ac:{uset, putPaper, takePaper}] keep(openStatus)
...
# tray coordination
  ...
  [ac:{opend,close,start,stop,checkA,checkB,checkC,jam}] keep(tray1.empty,tray1.openStatus)
  [start] keep(tray1.openStatus)
  makecopy <-> tray1.uset
  per(tray1.opent) -> door=open
  per(tray1.closet) -> door=open
  [ac:{tray1.opent,tray1.closet}] keep(door,copying,error) #,display.info)
...
# the display can only change in response to some event on the device
  [display.setDisplay(m)] (opend' | close' | start' | stop' | checkA' | checkB' | checkC' | jam' | makecopy' | tray1.opent' | tray1.closet' | tray1.uset' | tray1.putPaper' | tray1.takePaper')
